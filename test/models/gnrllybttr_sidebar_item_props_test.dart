  ...
  group('Given an instance of GnrllyBttrDecoration', () {
    group('When created with default values', () {
      ...
      test('Then common decoration should have default values', () {
        expect(decoration.common.animationDuration,
      ...
      test('Then scaffold decoration should have default values', () {
        expect(decoration.scaffold.backgroundColor,
            equals(const Color(0xFF18181B)));
        expect(decoration.scaffold.inset, isFalse);
        expect(decoration.scaffold.padding, equals(const EdgeInsets.all(16.0)));
        expect(decoration.scaffold.borderRadius,
      ...
      test('Then sidebarItem map should contain both positions', () {
        ...
        expect(
            decoration.sidebarItem.containsKey(SidebarPosition.left), isTrue);
        expect(
            decoration.sidebarItem.containsKey(SidebarPosition.right), isTrue);
      });
    });
    ...
    group('When created with custom values', () {
      ...
      final customSidebarItem =
          <SidebarPosition, GnrllyBttrSidebarItemDecoration>{
      ...
      test('Then it should hold the custom sidebar decorations', () {
        expect(decoration.sidebar[SidebarPosition.left]?.backgroundColor,
            equals(Colors.red));
        expect(decoration.sidebar[SidebarPosition.right]?.backgroundColor,
            equals(Colors.green));
      });

      test('Then it should hold the custom sidebarItem decorations', () {
        expect(decoration.sidebarItem[SidebarPosition.left]?.badgeColor,
            equals(Colors.purple));
        expect(decoration.sidebarItem[SidebarPosition.right]?.badgeColor,
    ...
    group('When created with invalid maps', () {
      ...
      test(
          'Then it should throw assertion error for incomplete sidebarItem map',
          () {
        expect(
          () => GnrllyBttrDecoration(
            sidebarItem: const <SidebarPosition,
                GnrllyBttrSidebarItemDecoration>{
